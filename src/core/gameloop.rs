...
/// A schedule that runs at every iteration of the event loop, but before [`IterStep`].
/// It is meant to be used in by the core of the app, to keep track of when the event queues should be cleared.
#[derive(ScheduleLabel, Clone, Debug, PartialEq, Eq, Hash)]
pub struct EventsCore;

/// The schedule that runs at every iteration of the event loop.
/// (To be more precise every time a redraw is requested, which might be several thousand times per second)
#[derive(ScheduleLabel, Clone, Debug, PartialEq, Eq, Hash)]
pub struct IterStep;

/// The schedule that runs at a fixed timestep, meant for game logic, physic updates, etc
#[derive(ScheduleLabel, Clone, Debug, PartialEq, Eq, Hash)]
pub struct Update;

/// The schedule that runs at a semi-fixed timestep, meant for things that should happen right before rendering
#[derive(ScheduleLabel, Clone, Debug, PartialEq, Eq, Hash)]
...
fn schedule_game_iteration(world: &mut World) {
	...
	// Call the fast-looping schedules at the beginning, so they don't delay delta_iteration in case they take longer than they should
	...
	// Update deltaTimes and update the accumulators that update() and render() can consume from
	// dt_f is either "constant" or basically equal to delta_iteration
	...
	for _ in 0..num_updates {
		...
		// TODO Somehow handle the case where update takes consistently longer than 16ms to run
	}

	if num_updates >= 1 {
		// Update UPS info; technically the UPS should be equal to target_ups, but since that's useless info,
		// predict the number of updates for the next second (very jittery), and let smooth_ups do its thing
	...
	if time.target_fps.is_some() {
		// If there is a certain target fps, then check that the accumulator has produced enough time that render() can consume
		if time.render_accumulator >= time.dt_f {
			should_render = true;

			time.render_accumulator -= time.dt_f;

			// In case FPS is consistently under the target, make sure that render_accumulator doesn't accumulate too much (which would compensate when FPS start raising again)
